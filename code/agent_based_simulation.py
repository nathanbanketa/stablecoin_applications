...
print("="*80)
print("STEP 7: Interpreting simulation results...")
print("="*80)
print()

print("WHAT THE SIMULATION SHOWS:")
print("-"*80)
print()

for country in sorted(simulation_results.keys()):
    results = simulation_results[country]
    actual = results['actual']
    baseline_sim = results['baseline']['final_adoption_rate']
    error = abs(actual - baseline_sim)
    
    print(f"{country}:")
    print(f"  Actual adoption: {actual:.1f}%")
    print(f"  Baseline simulation: {baseline_sim:.1f}%")
    print(f"  Error: {error:.1f} percentage points")
    
    if error < 5:
        print(f"  ✓ Close match - model captures adoption well")
    elif error < 10:
        print(f"  ~ Moderate match - model captures general trend")
    else:
        print(f"  ✗ Poor match - model missing important factors")
        print(f"    Interpretation: Non-cost infrastructure barriers present")
...
print("Use this for:")
print()
print("1. MECHANISM UNDERSTANDING")
print("   'Our agent-based model simulates individual adoption decisions.")
print("   [Country] shows close match between simulation and actual data,")
print("   suggesting [inflation/infrastructure] are key drivers.'")
print()

print("2. SCENARIO ANALYSIS")
print("   'If inflation increases by 50% (high inflation scenario), simulated")
print("   adoption would reach [X]%, compared to actual [Y]%.'")
print()

print("3. IDENTIFYING BARRIERS")
print("   'Poor model fit in [Country] suggests factors beyond inflation,")
print("   internet, and mobile subscriptions. Design interventions must")
print("   address [specific barriers from interviews].'")
print()

print("4. INTERVIEW VALIDATION")
print("   'Our simulation predicts adoption should be [X]%, but actual is [Y]%.")
print("   What explains this gap? What barriers do users face?'")
print()

print("="*80)
print("AGENT-BASED SIMULATION COMPLETE")
print("="*80)
print()
print("Output files:")
print("  ✓ simulation_actual_vs_predicted.png (results overview)")
print("  ✓ simulation_scenarios.png (different scenarios visualized)")
print("  ✓ simulation_comparison.csv (detailed results)")
print()
